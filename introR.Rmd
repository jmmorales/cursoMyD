...
# si queremos calcular el "factor reproductivo" o la 
# "tasa finita de incremento la poblaci?n
# tenenos que calcular n(t+1) / n(t)
# por ejemplo, para el 2010 la tasa de crecimiento fue
n[2]/n[1]

# para no repetir las cuentas a?o por a?o podemos hacer un loop
# pero antes necesitamos generar un vector para guardar los resultados

tasa <- numeric(length(n) - 1)
# aqu? combinamos dos funciones: 'numeric' y 'length'
# para ver qu? hace 'numeric' podemos hacer
?numeric

# poniendo un signo de pregunta y luego el nombre de una funci?n 
# accedemos a la documentaci?n de esa funci?n
# una mejor forma de encontrar ayuda e inlcuso informaci?n acerca de 
# funciones que no conocemos es visitando a http://rseek.org/

# volviendo a nuestro loop:
for(i in 2:length(n)){
  tasa[i-1] <- n[i]/n[i-1]
} 

# el comando 'for' aqu? itera en valores de 'i' la cuenta n(t+1)/n(t)
# fijense que 'i' est? definido de 2 hasta el largo del vector 'n'
2:length(n)
# usamos el valor de 'i' para cada iteraci?n para ver qu? valores de 'n' usar
# en la cuenta y para indicar en qu? posici?n del vector 'tasa' guardamos el resultado

# alternativamente podr?amos hacer
...
# para ver qu? sali? de estas cuentas escribimos
tasa

# para graficar la tasa de incremento en cada a?o hacemos
plot(yr[2:length(n)], tasa)

# podemos "embellecer" un poco el gr?fico usando 'main', 'xlab', y 'pch')
plot(yr[2:length(n)], tasa, main="Tasa de Crecimiento", xlab="a?o", pch=16)

# ahora que aprendimos a usar un 'for' loop,
# podemos ver que en este caso no lo necesit?bamos! 

tasa <- n[2:6]/n[1:5]
# esto es posible porque muchas operaciones en R est?n "vectorizadas",
# de manera que es m?s f?cil escribir y m?s r?pido ejecutar algunas cosas

# En cualquier caso, vemos que la tasa de incremento poblacional 
# es constante e igual a 3.
# Podemos usar ese valor para proyectar el tam?o poblacional en el futuro:
...
# lo mismo podr?amos hacer definiendo condiciones iniciales
# y usando la f?rmula n(t) = n(0)*lambda^t
...
# comprobemos que efectivamente da lo mismo
plot(n,nt)
...
# Es poco creible que la poblaci?n siga y siga creciendo como
# n(t) = n(0)*lambda^t
# Una opci?n es pensar que la tasa de incremento poblacional decrece 
# con el tama?o poblacional (modelo log?stico)

lambda <- 1.8
N <- 0:100
K <- 80  # capacidad de carga
rd <- lambda - 1 # rd es la tasa de crecimiento poblacional per c?pita
...
# Un aspecto clave de la programaci?n es escribir funciones.
# Una funci?n es un conjunto de instrucciones que toma "inputs" o "argumentos",
# usa esos inputs para calcular otros valores que devuelve como resultado.
# Por ejemplo, podemos escribir una funci?n para el modelo log?stico:
...
# esta funci?n tiene como argumentos a n0, rd, K y el n?mero de a?os 
# (nyr) que queremos simular.
#
# A menos que le digamos otra cosa, R respeta el orden en que definimos los 
# inputs de la funci?n. Entonces, si queremos simular la trayectoria de una 
# poblaci?n que sigue un modelo log?stico con n0=1, rd=1.5, y K=100 
# por 10 generaciones, podemos hacer:
n <- logistic(1,1.5,100)

# y graficar
plot(n)

# en este caso, no le dijimos a la funci?n que quer?amos nyr=10 porque
# est? definido por defecto. 

# En realidad no es muy buena idea pasarle valores de esta manera a las funciones
# Nos conviene ser m?s expl?citos:

n <- logistic(n0=1, rd=1.5, K=100, nyr=20)

# incluso podemos cambiar el orden de los argumentos si es que 
# los llamamos por su nombre:
n <- logistic(nyr=20, rd=1.5, n0=2, K=100)

# Otras veces conviene definir los valores de los inputs aparte
# por ejemplo:
